    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        if m.reply_to_message is not None and m.reply_to_message.document:
            if m.reply_to_message.document.file_name != "YouTube_PlayList.json":
                k=await m.reply("Invalid PlayList file given. Use @musikkugroup  or search for a playlist in @musikkuchannel to get a playlist file.")
                await delete_messages([m, k])
                return
            ...
            k=await m.reply("No playList file given. Use @musikkugroup  or search for a playlist in @musikkuchannel to get a playlist file.")
            await delete_messages([m, k])
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/musikkuchannel')]]))
    await delete_messages([m, k])
    ...
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                ...
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/escape_aja'),
            ]
            ]
